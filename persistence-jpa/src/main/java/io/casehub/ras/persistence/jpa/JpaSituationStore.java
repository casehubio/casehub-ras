...
public class JpaSituationStore implements SituationStore {
    ...
    public Optional<SituationContext> find(String situationId, String correlationKey,
        ...
        return Optional.of(mapper.toContext(entity));
    }
    ...
    public SituationContext save(SituationContext context) {
        ...
        try {
            if (existing != null) {
                ...
                em.flush();
                return context.withStoreVersion(existing.getVersion());
            } else {
                SituationEntity newEntity = mapper.toEntity(context);
                em.persist(newEntity);
                em.flush();
                return context.withStoreVersion(newEntity.getVersion());
            }
    ...
    public int removeExpired(Instant cutoff) {
        return em.createQuery("DELETE FROM SituationEntity s WHERE s.lastSignal <= :cutoff")
    ...
    @Override
    @Transactional(TxType.REQUIRED)
    public int removeTriggeredBefore(Instant triggerCutoff) {
        return em.createQuery("DELETE FROM SituationEntity s WHERE s.policyTriggered = true " +
    ...
    @Override
    @Transactional(TxType.REQUIRED)
    public List<SituationContext> findActive(String tenancyId) {
