...
- Per-ganglion quality metrics: `docs/specs/2026-08-21-per-ganglion-quality-metrics-design.md`
- Feedback quality enrichment (drift, per-ganglion recall, cross-ref): `docs/specs/issue-62-drift-recall-quality/2026-08-29-feedback-quality-enrichment-design.md`
...
| `GanglionContribution` | Per-ganglion outcome attribution — `ganglionId`, `confidence`, `signal`. Stored as JSONB on `OutcomeRecord`. One entry per unique ganglionId, deduplicated at recording time (highest signal, confidence tiebreaker). |
| `GanglionOutcomeStatistics` | Per-ganglion aggregate — `ganglionId`, `totalOutcomes`, `noiseCount`, `confirmedCount`, `neutralCount`, `missedCount`. Implements `QualityMetrics`. `recall()` method (NaN when missedCount==0). Only counts positive-signal (DETECTED/WEAK) contributions for outcome metrics; missedCount from ganglion-attributed missed detection reports. |
| `QualityMetrics` | Shared interface for quality computation — `precision()` and `noiseRate()` default methods. Implemented by `OutcomeStatistics` and `GanglionOutcomeStatistics`. |
| `FeedbackConfig` | Feedback loop configuration on `SituationDefinition` — `noiseLabels`, `confirmedLabels` (disjoint sets), `suppressionCooldown` (positive Duration), `learningRate` (0.0–1.0], `retentionPeriod` (>= cooldown), `tuningEnabled` (default false), `overSensitiveThreshold` (default 0.5), `underSensitiveThreshold` (default 0.5), `crossRefWindow` (default PT1H). `classify(String outcomeLabel)` returns `OutcomeClassification`. 6-arg backwards-compat constructor defaults new fields. |
| `DriftDirection` | Enum — OVER_SENSITIVE, UNDER_SENSITIVE, BOTH_DRIFTING, STABLE, INSUFFICIENT_DATA. Classified by `FeedbackTuningStrategy.classifyDrift()`. Published as `ras.feedback.drift` state-gauge set. BOTH_DRIFTING suppresses auto-tuning in `FeedbackUpdateJob`. |
| `OutcomeClassification` | Enum — NOISE, CONFIRMED, NEUTRAL. Derived from case outcome label via `FeedbackConfig.classify()`. |
| `OutcomeRecord` | Immutable record — `situationId`, `correlationKey`, `tenancyId`, `outcomeLabel`, `classification`, `closedAt`, `caseId`, `List<GanglionContribution> ganglionContributions`. UNIQUE(case_id) for at-least-once dedup. 7-arg backwards-compat constructor defaults contributions to `List.of()`. |
...
| `OutcomeLedger` | SPI — `record()`, `statistics()`, `lastNoiseDismissalTime()`, `countByLabel()`, `ganglionStatistics()`, `distinctTenancies()`, `removeRecordsBefore()`. `ganglionStatistics()` returns `Map<String, GanglionOutcomeStatistics>` — per-ganglion breakdown filtering to positive signals (DETECTED/WEAK). `InMemoryOutcomeLedger` (`@DefaultBean` in runtime/), `JpaOutcomeLedger` (`@ApplicationScoped` in persistence-jpa/). |
...
| `FeedbackTuningStrategy` | SPI — `adjustThreshold(OutcomeStatistics, currentThreshold, FeedbackConfig)`, `adjustPriors(currentPriors, outcomeCounts, FeedbackConfig)`, `classifyDrift(OutcomeStatistics, FeedbackConfig)` default method returning `DriftDirection`. `DefaultTuningStrategy` (`@DefaultBean`) uses configurable noise-rate threshold (`config.overSensitiveThreshold()`) and Laplace-smoothed prior blending. |
