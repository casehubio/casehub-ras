...
| Module | Purpose |
|--------|---------|
| `api` | `Ganglion` SPI, `SituationStore` SPI, `RasTriggerPolicy` SPI, `DetectionResult`, `SituationContext`, `ChainMode`, `SituationDefinition` |
| `persistence-memory` | `InMemorySituationStore` — ConcurrentHashMap-backed, zero-config default |
...
| `testing` | `MockGanglion`, `FixedDetectionResult` — test fixtures |
